    ...
    <div className="space-y-3 md:space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 md:gap-4 px-1 md:px-2">
        ...
        <div className="flex items-center gap-1 md:gap-2 bg-white px-2.5 py-1.5 md:px-4 md:py-2.5 rounded-lg md:rounded-2xl border border-slate-100 shadow-sm w-fit">
          ...
          <select
            className="text- md:text- font-black uppercase tracking-widest text-slate-600 outline-none bg-transparent cursor-pointer"
            value={filterStatus}
      ...
      {filteredOrders?.length > 0? filteredOrders.map((order) => {
        ...
        return (
          <div key={order?.id} className="bg-white border border-slate-100 rounded-xl md:rounded- p-3 md:p-10 shadow-sm overflow-hidden relative">

            <div className="flex flex-col md:flex-row justify-between gap-2 md:gap-8 mb-5 md:mb-12">
              <div>
                <p className="text- md:text- font-black text-blue-600 uppercase tracking-[0.3em] mb-0.5 md:mb-2">
              ...
              <div className="text-left md:text-right">
                 <p className="text-sm md:text-2xl font-[1000] text-slate-900">
                   {(order?.amount || 0).toLocaleString()} <span className="text- md:text-xs text-slate-400">CFA</span>
                 </p>
                 <p className="text- md:text- font-bold text-slate-400 uppercase tracking-widest mt-0.5 md:mt-1">
              ...
              {STEPS.map((step, index) => {
                ...
                return (
                  <div key={index} className="relative z-10 flex flex-col items-center gap-1.5 md:gap-3 bg-white px-1 md:px-2">
                    <div className={`w-6 h-6 md:w-10 md:h-10 rounded-full flex items-center justify-center border- md:border-4 transition-all duration-500
                    ...
                    <p className={`text- md:text- font-black uppercase tracking-widest text-center ${isCompleted? 'text-slate-900' : 'text-slate-300'}`}>
                      {step.label}
                    </p>
                  </div>
                );
            ...
            <div className="mt-5 md:mt-12 pt-3 md:pt-8 border-t border-slate-50 flex flex-col md:flex-row justify-between items-start md:items-center gap-2 md:gap-4">
               <div className="flex items-center gap-2 md:gap-3 w-full md:w-auto">
                  ...
                  <p className="text- md:text- font-black text-slate-600 uppercase tracking-tight truncate">
                    Livraison : <span className="text-slate-900 block sm:inline mt-0.5 sm:mt-0">{order?.delivery_address || 'Abidjan, Cocody'}</span>
                  </p>
               </div>
               <button className="flex items-center justify-center w-full md:w-auto gap-1 md:gap-2 text-blue-600 font-black text- md:text- uppercase tracking-widest hover:translate-x-1 md:hover:translate-x-2 transition-transform bg-slate-50 md:bg-transparent py-2 md:py-0 rounded-md md:rounded-none">
        ...
        <div className="bg-white rounded-xl md:rounded- p-6 md:p-20 text-center border-2 border-dashed border-slate-100">
           <Package className="w-6 h-6 md:w-12 md:h-12 text-slate-200 mx-auto mb-2 md:mb-4" />
           <p className="text- md:text- font-black text-slate-400 uppercase tracking-widest">
