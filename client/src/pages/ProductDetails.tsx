...
import {
  ...
  PenLine, X, Tag, Heart, ChevronDown, ChevronUp
} from 'lucide-react';
...
export default function ProductDetails() {
  ...
  // État pour gérer l'extension de la description
  ...
  const handleSubmitReview = async () => {
    if (!user) return navigate('/login'); if (!reviewComment.trim()) return;
    setIsSubmittingReview(true);
    await supabase.from('reviews').insert({ product_id: id, user_id: user.id, rating: reviewRating, comment: reviewComment.trim() });
    setShowReviewModal(false); setReviewComment(''); setReviewRating(5); await fetchProduct(); setIsSubmittingReview(false);
  };

  useEffect(() => { supabase.auth.getUser().then(({ data }) => setUser(data.user)); fetchProduct(); }, [id]);
  useEffect(() => { (async()=>{ if(!user||!id) return; const {data}=await supabase.from('favorites').select('id').eq('user_id',user.id).eq('product_id',id).maybeSingle(); setIsFavorite(!!data); })(); }, [user,id]);
  ...
  // Composant local pour les avis pour éviter la répétition
  const ReviewsSection = () => (
    <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
      <div className="flex items-center justify-between mb-4 pb-2 border-b">
        <h3 className="text-sm font-black uppercase">Avis Clients ({reviewsArray.length})</h3>
        <button onClick={()=>setShowReviewModal(true)} className="text-xs font-bold text-blue-600 uppercase flex items-center gap-1 bg-blue-50 px-2 py-1 rounded"><PenLine size={12}/>Rédiger</button>
      </div>
          ...
          {reviewsArray.slice(0, 3).map((r: any, i: number)=>(
            <div key={i} className="bg-slate-50 p-3 rounded-lg border border-slate-100">
              <div className="flex justify-between items-center mb-1">
                <p className="text-[11px] font-bold uppercase flex items-center gap-1 text-slate-700"><UserCircle2 size={14} className="text-slate-400"/>{r.profiles?.store_name||'Client'}</p>
                <div className="flex gap-0.5">{[1,2,3,4,5].map(s=><Star key={s} size={10} fill={s<=(r.rating||4)?"#FACC15":"none"} className={s<=(r.rating||4)?"text-amber-400":"text-slate-200"}/>)}</div>
              </div>
    ...
    <div className="bg-slate-50 min-h-screen pb-10 pt-4">
      <div className="max-w-6xl mx-auto px-4">
        ...
        <div className="flex justify-between mb-4">
          <button onClick={()=>navigate(-1)} className="flex items-center gap-1.5 text-sm font-bold uppercase bg-white px-3 py-2 rounded-lg border border-slate-200 hover:bg-slate-50 transition-colors"><ArrowLeft size={14}/>Retour</button>
          <button onClick={handleToggleFavorite} className={`flex items-center gap-1.5 text-sm font-bold uppercase px-3 py-2 rounded-lg border transition-colors ${isFavorite?'bg-red-50 text-red-600 border-red-200':'bg-white text-slate-500 border-slate-200'}`}><Heart size={14} fill={isFavorite?"currentColor":"none"}/>{isFavorite?'Retirer':'Favoris'}</button>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          
          {/* COLONNE GAUCHE : IMAGE + AVIS (DESKTOP) */}
          <div className="lg:col-span-7 flex flex-col gap-6">
            <div className="bg-white p-4 rounded-xl border border-slate-200 relative flex items-center justify-center min-h-[300px] shadow-sm">
              ...
              <div className={`absolute top-3 left-3 px-2 py-1 rounded text-[10px] font-black uppercase text-white flex items-center gap-1 border shadow-sm ${isOfficial?'bg-blue-900 border-blue-500/50':'bg-slate-900'}`}>{isOfficial?<Crown size={12}/>:<Store size={12}/>}{storeName}</div>
            </div>

            {/* Avis visible uniquement sur ordinateur ici */}
            <div className="hidden lg:block">
              <ReviewsSection />
            </div>
          </div>

          {/* COLONNE DROITE : INFOS + DESCRIPTION + AVIS (MOBILE) */}
          <div className="lg:col-span-5 flex flex-col gap-4">
            ...
            <div className="space-y-2">
              <button onClick={()=>{addToCart(productWithFinalPrice);navigate('/checkout')}} className="w-full bg-blue-600 hover:bg-blue-700 text-white py-3.5 rounded-xl text-sm font-black uppercase flex items-center justify-center gap-2 transition-all shadow-lg shadow-blue-200"><ShoppingCart size={18}/>Acheter maintenant</button>
              <div className="grid grid-cols-2 gap-2">
                <a href={whatsappUrl} target="_blank" rel="noreferrer" className="bg-white border-2 border-emerald-100 hover:bg-emerald-50 text-emerald-600 py-2.5 rounded-xl text-xs font-bold uppercase flex items-center justify-center gap-1.5 transition-colors"><MessageCircle size={16}/>WhatsApp</a>
                <button onClick={handleToggleFavorite} className={`border-2 py-2.5 rounded-xl text-xs font-bold uppercase flex items-center justify-center gap-1.5 transition-colors ${isFavorite?'bg-red-50 text-red-600 border-red-100':'bg-white text-slate-700 border-slate-200 hover:bg-slate-50'}`}><Heart size={16} fill={isFavorite?"currentColor":"none"}/>Favoris</button>
              </div>
            </div>
            ...
            <div className="bg-slate-900 p-4 rounded-xl text-white flex items-center gap-4 shadow-md">
              ...
              <div><p className="text-[10px] text-slate-400 uppercase font-bold">Certification SpaceAuto</p><h4 className="text-sm font-black uppercase">Produit Garanti</h4></div>
            </div>

            {/* DESCRIPTION AVEC FIX "PAGE BLANCHE" */}
            {fullDescription && (
              <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                <h3 className="text-xs font-black uppercase text-slate-400 mb-2">Description Technique</h3>
            ...
            {/* Avis visible uniquement sur mobile ici */}
        ...
        <div className="mt-10 pt-10 border-t border-slate-200">
          <h2 className="text-lg font-black uppercase text-slate-800 mb-6 flex items-center gap-2"><Tag className="text-blue-600" size={20}/>Produits suggérés</h2>
      ...
      {showReviewModal && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl">
            <div className="flex justify-between items-center mb-6"><h3 className="font-black uppercase text-slate-800">Donner votre avis</h3><button onClick={()=>setShowReviewModal(false)} className="p-1.5 bg-slate-100 rounded-full hover:bg-slate-200"><X size={18}/></button></div>
            <div className="flex gap-2 mb-6 justify-center">
              {[1,2,3,4,5].map(s=><button key={s} onClick={()=>setReviewRating(s)} className="transform hover:scale-110 transition-transform"><Star size={32} fill={s<=reviewRating?"#FACC15":"none"} className={s<=reviewRating?"text-amber-400":"text-slate-200"}/></button>)}
            </div>
            <textarea value={reviewComment} onChange={e=>setReviewComment(e.target.value)} rows={4} className="w-full border border-slate-200 rounded-xl p-3 text-sm mb-6 focus:ring-2 focus:ring-blue-500 outline-none" placeholder="Votre expérience avec ce produit..."/>
            <button onClick={handleSubmitReview} disabled={isSubmittingReview} className="w-full bg-blue-600 hover:bg-blue-700 text-white py-3 rounded-xl text-sm font-bold uppercase shadow-lg shadow-blue-200 disabled:opacity-50">Publier mon avis</button>
          </div>
        </div>
      )}
    </div>
