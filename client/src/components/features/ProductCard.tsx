import { ShoppingCart, Image as ImageIcon, MapPin, Settings, Star, ShieldCheck, Hash, Car } from 'lucide-react';
...
export default function ProductCard({ product }: ProductCardProps) {
  ...
  const getWhatsAppLink = () => {
    ...
    const message = encodeURIComponent(`Bonjour, je suis intéressé par l'article "${product?.name}" (Réf: ${product?.reference || 'N/A'}) affiché à ${finalPrice.toLocaleString('fr-FR')} FCFA sur SpaceAuto24.`);
  ...
  return (
    <div className="bg-white border border-slate-200/80 shadow-sm hover:shadow-xl transition-all duration-500 flex flex-col h-full group rounded-xl md:rounded-2xl overflow-hidden relative">
      ...
      <div className="p-2.5 md:p-5 flex flex-col flex-grow bg-white">
        ...
        {/* 🟢 BADGES TECHNIQUES PRO (Référence & Modèle) */}
